...
export class RoomDetailAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.serviceRoom.getUnavailableDays(this.roomId).subscribe((res: any) => {
   ...
   this.suggestions = [
    {
      ...
      "rules": [
        "No smoking.",
        "Check-in time from 2 PM, check-out by 12 AM.",
        "Car parking"
      ],
      "title": "Amber Star",
  ...
  submitReservation(): any {
    this.serviceRoom.getRoomWithKey(this.roomId).subscribe((room: any) => {
      ...
      this.serviceRoom.AddToCart(RoomWithDate);
    });

    this.routerNavigation.navigate(['/orders-a']);
  }

  AccessRoom(key: any): void {
